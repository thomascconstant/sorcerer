...
fileQuestions = "data/xp1_questionnaire.csv" #résultats questionnaire XP CN2 avec diff DDA
...
```{r echo=FALSE}
#ggplot(DTM, aes(group=mise,x=mise, y=error.subj.diff.mise)) + 
#  geom_boxplot(outlier.shape=NA) +
#  geom_jitter(position=position_jitter(width=.3, height=0, alpha=0.2))
...
```{r players.level.analysis.functions, echo=FALSE} 
level.between.games <- function(DT){
  
  
  
}

level.gamer.profile <- function(DT){
  p <- ggplot(DT, aes(x=play.video.games, y=niveau)) + geom_point(alpha=0.1)
  print(p)  
  
  
  DT$play.video.games.group = 0
  DT[play.video.games > 0.6 , play.video.games.group := 1]
  
  DT = DT[,.(niveau=mean(niveau),play.video.games=mean(play.video.games)),by=.(play.video.games.group,IDjoueur)]
  
  
  
  res = DT[,.(nb=nrow(.SD)),by=play.video.games.group]
  print(res)
  
  #chisq.test(x=DT$play.video.games.group,y=DT$niveau.group)
  
  #print(wilcox.test(DT[play.video.games.group == 0]$niveau, mu=0, conf.int=T))
  #print(wilcox.test(DT[play.video.games.group == 1]$niveau, mu=0, conf.int=T))
  
  print(wilcox.test(DT[play.video.games.group == 0]$niveau,DT[play.video.games.group == 1]$niveau, conf.int=T))
  
  p = ggplot(DT, aes(factor(play.video.games.group), niveau)) + geom_boxplot()
  print(p)
...
```{r players.level.analysis, echo=FALSE} 
level.gamer.profile(DTM)
level.gamer.profile(DTS)
level.gamer.profile(DTL)
