...
plotCurves <- function (numStep,diffSubj,diffObj,confiance,id){
  #print(diffSubj);
  df <- data.frame(ns=numStep,do=diffObj,ds=diffSubj,c=confiance/3)
  ...
  pl <- pl + geom_line(aes(x=ns,y=confiance/3),size=0.5,colour="#229922");
...
```{r echo=FALSE}
if(plotDiffCurves){
  void <- DTL[,{plotCurves(action_de_jeu,evalDiff,estDiff,resLisse,IDjoueur);NULL},by=.(IDjoueur)]
}else{
...
```{r echo=FALSE}
if(plotDiffCurves){
  void <- DTM[,{plotCurves(action_de_jeu,evalDiff,estDiff,resLisse,IDjoueur);NULL},by=.(IDjoueur)]
}else{
  print("plotDiffCurves == FALSE")
}
```
...
```{r echo=FALSE}
if(plotDiffCurves){
  void <- DTS[,{plotCurves(action_de_jeu,evalDiff,estDiff,resLisse,IDjoueur);NULL},by=.(IDjoueur)]
}else{
  print("plotDiffCurves == FALSE")
}
```
