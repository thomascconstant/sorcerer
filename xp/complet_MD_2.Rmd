...
DTDesc = rbind(DTDesc,DTS[,.(type="Sensorielle",sdMise=sd(miseNorm)),by=IDjoueur])
...
print(p)
outliers <- boxplot.stats(DTDesc$sdMise)$out
print(paste("Out:",outliers))
```
...
```{r echo=FALSE}
DTDesc = data.table();
DTDesc = DTM[,.(type="Moteur",mMise=mean(miseNorm)),by=IDjoueur]
DTDesc = rbind(DTDesc,DTS[,.(type="Sensorielle",mMise=mean(miseNorm)),by=IDjoueur])
DTDesc = rbind(DTDesc,DTL[,.(type="Logique",mMise=mean(miseNorm)),by=IDjoueur])
p <- ggplot(DTDesc, aes(type,mMise))
p <- p + geom_boxplot() + geom_point(shape=1) 
print(p)
outliers <- boxplot.stats(DTDesc$mMise)$out
print(paste("Out:",outliers))
```
...
```{r echo=FALSE}
DTDesc = data.table();
DTDesc = DTM[,.(type="Moteur",dConst=sum(gagnant)),by=IDjoueur]
DTDesc = rbind(DTDesc,DTS[,.(type="Sensorielle",dConst=sum(gagnant)),by=IDjoueur])
DTDesc = rbind(DTDesc,DTL[,.(type="Logique",dConst=sum(gagnant)),by=IDjoueur])
p <- ggplot(DTDesc, aes(type,dConst))
p <- p + geom_boxplot() + geom_point(shape=1) 
print(p)
outliers <- boxplot.stats(DTDesc$dConst)$out
print(paste("Out:",outliers))
```
