...
plot.objective.diff.smooth <- function (DT) {
  p <- ggplot(DT, aes(action_de_jeu, obj.diff)) + geom_point(alpha=0.5) + geom_smooth() + xlab("Time (turns)") + ylab("Objective Difficulty") + theme(text = element_text(size=20))
  print(p)
}
```
...
DTAll$error.subj.diff.mise = abs(DTAll$subj.diff.mise - DTAll$obj.diff)
...
```{r modeling.subjective.difficulty.time, echo=FALSE}   
#---------------------------------------------------------------------
#        MODELING TIME INFLUENCE ON SUBJECTIVE DIFFICULTY 
#---------------------------------------------------------------------
#adding error between subjective difficulty and objective difficulty
DTAll$subj.diff.mise = 1 - DTAll$miseNorm;
DTAll$error.subj.diff.mise.time = abs(DTAll$subj.diff.mise - DTAll$timeNorm)
DTAll <- DTAll[,error.subj.diff.mise.time.mean:=mean(error.subj.diff.mise.time),by=IDjoueur]
...
for(i in 2:nrow(DTAll)){
  if(id[i] != lastID){
    ...
    if(gagnant[i-1]){
      cur.confidence = timeNorm[i-1];
    }else{
      cur.confidence = -(1 - timeNorm[i-1]);
    }
    cur.confidence.liss <- alpha * cur.confidence.liss + cur.confidence;
  }
  lastID <- id[i];
...
plot.subjective.difficulty <- function(DT,selGroup){
...
plot.subjective.difficulty(DTAll,"all")
```

##Motor task

```{r plot.subjective.difficulty.motor, echo=FALSE}
plot.subjective.difficulty(DTM,"all")
plot.subjective.difficulty(DTM,"good")
plot.subjective.difficulty(DTM,"medium")
plot.subjective.difficulty(DTM,"bad")
```

##Sensory task

```{r plot.subjective.difficulty.sensory, echo=FALSE}
plot.subjective.difficulty(DTS,"all")
plot.subjective.difficulty(DTS,"good")
plot.subjective.difficulty(DTS,"medium")
plot.subjective.difficulty(DTS,"bad")
```

##Logical task

```{r plot.subjective.difficulty.logical, echo=FALSE}
plot.subjective.difficulty(DTL,"all")
plot.subjective.difficulty(DTL,"good")
plot.subjective.difficulty(DTL,"medium")
plot.subjective.difficulty(DTL,"bad")
```

```{r plot.subjective.objective.difficulty.time, echo=FALSE}   
#------------------------------------------------------
#     SHOWING SUBJECTIVE VS TIME 
#------------------------------------------------------
...
plot.subjective.difficulty.time <- function(DT,selGroup){
  ...
  # Lien entre mise normalisée et temps (learning effect)
  ...
  for(i in 1:nb.bins){
    ...
    DTLoc = DTLoc[,.(mise.mean=mean(subj.diff.mise)),by=IDjoueur]
    subj.diff = DTLoc$mise.mean
    timeNorm.bin[i] = timeNorm.bin.cur
    subj.diff.med[i] = NA
    test.pvals[i] = NA
    conf.min[i] = NA
    conf.max[i] = NA
    delta.time.subj[i] = NA
    shapes[i] = 16
    nb.vals[i] = length(subj.diff)
    if(nb.vals[i] > 1){
      ...
      if (class(try.res) != "try-error"){
        ...
        conf.min[i] = test.res$conf.int[1]
        conf.max[i] = test.res$conf.int[2]
        delta.time.subj[i] = signif(subj.diff.med[i] - timeNorm.bin.cur,digit=2)
      }
    }
  }
...
```{r plot.subjective.difficulty.time.all, echo=FALSE}
plot.subjective.difficulty.time(DTAll,"all")
```

##Motor task

```{r plot.subjective.difficulty.time.motor, echo=FALSE}
plot.subjective.difficulty.time(DTM,"all")
plot.subjective.difficulty.time(DTM,"good")
plot.subjective.difficulty.time(DTM,"medium")
plot.subjective.difficulty.time(DTM,"bad")
```

##Sensory task

```{r plot.subjective.difficulty.time.sensory, echo=FALSE}
plot.subjective.difficulty.time(DTS,"all")
plot.subjective.difficulty.time(DTS,"good")
plot.subjective.difficulty.time(DTS,"medium")
plot.subjective.difficulty.time(DTS,"bad")
```

##Logical task

```{r plot.subjective.difficulty.time.logical, echo=FALSE}
plot.subjective.difficulty.time(DTL,"all")
plot.subjective.difficulty.time(DTL,"good")
plot.subjective.difficulty.time(DTL,"medium")
plot.subjective.difficulty.time(DTL,"bad")
```
