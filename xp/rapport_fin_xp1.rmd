...
plot.objective.diff.model.curves <- function (DT,fit) {
  ...
  for(i in 1:nrow(DTLevel)){
    ...
    if(i==1){
      p = ggplot(DT,aes(x=difficulty,y=estDiff,colour=group.color)) + geom_path(alpha = 0.2) + xlim(0,1) + ylim(0,1)
    } else {
...
summary(glm(error.subj.diff.mise~action_de_jeu + obj.diff,data = DTM[niveau.group=="bad"]))
plot.subjective.diff.error.density(DTM[niveau.group=="medium"])
summary(glm(error.subj.diff.mise~action_de_jeu + obj.diff,data = DTM[niveau.group=="medium"]))
plot.subjective.diff.error.density(DTM[niveau.group=="good"])
summary(glm(error.subj.diff.mise~action_de_jeu + obj.diff,data = DTM[niveau.group=="good"]))
