...
# ```{r removing.outliers.setup.bet, echo=FALSE}
# #------------------------------------------------------
# #            OUTLIERS SETUP
# #------------------------------------------------------
# 
...
# ```
# 
# ```{r detect.outliers.win.sum.bet, echo=FALSE}
# #------------------------------------------------------
...
# ```
# 
# ```{r detect.outliers.sheeps.saved.bet, echo=FALSE}
# #------------------------------------------------------
...
# ```
# 
# ```{r detect.outliers.dda.exploit.bet, echo=FALSE}
# #------------------------------------------------------
...
# outliersLoc = get.outliers(DTDescM,DTDescS,DTDescL)
# outliers = merge(outliers,outliersLoc,by=c("id","type"),all=TRUE)
# print(paste("Outliers BET EXPLOIT DDA:",toString(outliersLoc$id)))
# 
...
# ```
# 
# ```{r detect.outliers.summary.bet, echo=FALSE}
# #------------------------------------------------------
# #            OUTLIERS SUMMARY
# #------------------------------------------------------
...
# ```
# 
# ```{r remove.outliers.bet, echo=FALSE}
# #------------------------------------------------------
...
```{r removing.outliers.setup.cs, echo=FALSE}
#------------------------------------------------------
#            OUTLIERS SETUP
#------------------------------------------------------
...
DTM[IDjoueur %in% unlist(outliers[type=="Moteur"]$id) ,{plot.diff.curve.cs(.SD,"Outlier CS Sd Motor Task");NULL},by=.(IDjoueur)]
...
DTM[IDjoueur %in% unlist(outliers[type=="Moteur"]$id) ,{plot.diff.curve.cs(.SD,"Outlier CS Null Motor Task");NULL},by=.(IDjoueur)]
DTS[IDjoueur %in% unlist(outliers[type=="Sensoriel"]$id) ,{plot.diff.curve.cs(.SD,"Outlier CS Null Sensory Task");NULL},by=.(IDjoueur)]
DTL[IDjoueur %in% unlist(outliers[type=="Logique"]$id) ,{plot.diff.curve.cs(.SD,"Outlier CS Null Logical Task");NULL},by=.(IDjoueur)]
```

```{r detect.outliers.win.sum.cs, echo=FALSE}
#------------------------------------------------------
#           OUTLIERS SUM OF WINS
#------------------------------------------------------
#Difficulty : win sum
...
```{r detect.outliers.sheeps.saved.cs, echo=FALSE}
 #------------------------------------------------------
 #           OUTLIERS SAVED SHEEPS
 #------------------------------------------------------
 #Difficulty and strategy = saved sheeps
 ...
 outliersLoc = get.outliers(DTDescM,DTDescS,DTDescL)
 outliers = merge(outliers,outliersLoc,by=c("id","type"),all=TRUE)
 print(paste("Outliers CS SAVED SHEEPS:",toString(outliersLoc$id)))
...
```{r detect.outliers.dda.exploit.cs, echo=FALSE}
#------------------------------------------------------
#           OUTLIERS EXPLOIT DDA
#------------------------------------------------------
#DDA Exploit : Win/Fail delta sum max
...
# outliersLoc = get.outliers(DTDescM,DTDescS,DTDescL)
# outliers = merge(outliers,outliersLoc,by=c("id","type"),all=TRUE)
# print(paste("Outliers CS EXPLOIT DDA:",toString(outliersLoc$id)))
# 
# DTM[IDjoueur %in% unlist(outliersLoc[type=="Moteur"]$id) ,{plot.diff.curve(.SD,"Outlier Delta Bet Motor Task");NULL},by=.(IDjoueur)]
# DTS[IDjoueur %in% unlist(outliersLoc[type=="Sensoriel"]$id) ,{plot.diff.curve(.SD,"Outlier Delta Bet Sensory Task");NULL},by=.(IDjoueur)]
# DTL[IDjoueur %in% unlist(outliersLoc[type=="Logique"]$id) ,{plot.diff.curve(.SD,"Outlier Delta Bet Logical Task");NULL},by=.(IDjoueur)]
...
```{r detect.outliers.summary.cs, echo=FALSE}
#------------------------------------------------------
#            OUTLIERS SUMMARY
#------------------------------------------------------
print(paste("Total number of outliers: ",toString(nrow(unique(outliers,by="id")))))
...
```{r remove.outliers.cs, echo=FALSE}
#------------------------------------------------------
#            REMOVING OUTLIERS FROM TABLES
#------------------------------------------------------
# removing all outliers and creating a new file only for Confidence Scale Outliers
DTM <- DTM[!IDjoueur %in% unlist(outliers[type=="Moteur"]$id)]
DTS <- DTS[!IDjoueur %in% unlist(outliers[type=="Sensoriel"]$id)]
DTL <- DTL[!IDjoueur %in% unlist(outliers[type=="Logique"]$id)]
...
for(i in 2:nrow(DTAll)){
  if(id[i] != lastID){
    ...
    cur.confidence.liss <- alpha * cur.confidence.liss + (1-alpha) * cur.confidence;
  }
  lastID <- id[i];
...
```{r plot.subjective.objective.difficulty.confidence.scale, echo=FALSE}   
#-------------------------------------------------------------------------------------
