---
title: "Experiment with Random Difficulty"
output:
  html_document:
    theme: lumen
...
#Plot la courbe de diff pour un joueur avec mise (bet)
...
#Plot la courbe de diff pour un joueur avec confiance (confidence scale)
plot.diff.curve.confidence.scale <- function (DT,title){
  if(nrow(DT) == 0)
    return()
  df <- data.frame(time=DT$action_de_jeu,
                   difficulty=DT$difficulty,
                   bet=DT$confianceNorm)
  p <- ggplot(df,aes(x=time)) 
  p <- p + geom_step(aes(y=bet,colour="Confidence scale"))
...
#------------------------------------------------------
# not useful for analysis based on CONFIDENCE SCALE
...
```{r detect.outliers.confidence.sd, echo=FALSE}
#------------------------------------------------------
#            OUTLIERS CONFIDENCE SCALE STD DEV
#------------------------------------------------------
...
plot.outliers(rbind(DTDescM,rbind(DTDescL,DTDescS)), "Confidence Standard Dev");

outliers = get.outliers(DTDescM,DTDescS,DTDescL)
print(paste("Outliers :",toString(outliers$id)))

DTM[IDjoueur %in% unlist(outliers[type=="Moteur"]$id) ,{plot.diff.curve.confidence.scale(.SD,"Outlier Confidence Sd Motor Task");NULL},by=.(IDjoueur)]
DTS[IDjoueur %in% unlist(outliers[type=="Sensoriel"]$id) ,{plot.diff.curve.confidence.scale(.SD,"Outlier Confidence Sd Sensory Task");NULL},by=.(IDjoueur)]
DTL[IDjoueur %in% unlist(outliers[type=="Logique"]$id) ,{plot.diff.curve.confidence.scale(.SD,"Outlier Confidence Sd Logical Task");NULL},by=.(IDjoueur)]
```
...
#------------------------------------------------------------------------
#        MODELING SUBJECTIVE DIFFICULTY (based on BET)
#------------------------------------------------------------------------
...
plot.subjective.difficulty <- function(DT){
  ...
  p = ggplot() +
        ...
        xlim(0,1)+
        ylim(0,1)+
        xlab("Objective difficulty levels") +
        ylab("Subjective difficulty levels") +
...
#title(main="Results for all tasks")
```
...
#title(main="Results for Motor task")
```
...
#title(main="Results for Sensitive task")
```
...
#title(main="Results for Logical task")
```
...
```{r test2, echo=FALSE} 
#------------------------------------------------------------------------------------------
#        MODELING SUBJECTIVE DIFFICULTY (based on CONFIDENCE SCALE /!\ not bet)
#------------------------------------------------------------------------------------------
#adding error between subjective difficulty and objective difficulty
