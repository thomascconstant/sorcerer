...
plot.objective.diff.model.curves <- function (DT,fit,time=1,groups=T) {
  ...
  p <- p + xlab("Variable de difficulté") + ylab("Difficulté objective") + theme(text = element_text(size=20))
...
maxConf <- 9 #confiance max avant normalisation (3 ca veut dire que si on perd 3 fois à diff 0 on a perdu confirance)
i = 1
for(i in 2:nrow(DTAll)){
  if(id[i] != id[i-1] || game[i] != game[i-1]){
    ...
    cur.confidence.liss <-  cur.confidence.liss + alpha * reward;
    cur.confidence.liss = sign(cur.confidence.liss) * min(maxConf,abs(cur.confidence.liss))
  }
...
DTAll$est.confidence.norm = exp(DTAll$est.confidence) / (1+exp(DTAll$est.confidence));#est.confidence
...
p <- test.profile(DTM$error.subj.diff.confiance,DTM$confianceNorm,"DTM")
...
cat("Résultat test Kendall erreur basée sur mise avec confiance estimée :", p)
p <- test.profile(DTS$error.subj.diff.confiance,DTS$confianceNorm,"DTS")
cat("Résultat test Kendall erreur basée sur échelle de confiance avec confiance estimée :", p)
...
cat("Résultat test Kendall erreur basée sur mise avec confiance estimée :", p)
p <- test.profile(DTL$error.subj.diff.confiance,DTL$confianceNorm,"DTL")
cat("Résultat test Kendall erreur basée sur échelle de confiance avec confiance estimée :", p)
