...
#                  GLOBAL VARIABLES
#------------------------------------------------------
g.ignore.confidence.scale.outilers <- TRUE
...
est.confidence[1] = 0;
alpha <- 7.0 #Vitesse de variation de la confiance
beta <- 0.6 #min modif on win or lose
maxConf <- 4 #confiance max avant normalisation (qu'elle se barre pas dans des extremes, meme si la normalisation rattrappe, elle bougera plus)
i = 1
for(i in 2:nrow(DTAll)){
  if(id[i] != id[i-1] || game[i] != game[i-1]){
    ...
    if(gagnant[i-1]){
      ...
      cur.confidence.liss <- cur.confidence.liss + 1
    }
...
DTAll$est.confidence.norm = DTAll$est.confidence;
...
```{r previous.diff.and.error.confidence.scale, echo=FALSE} 

#Plot la courbe de diff pour un joueur
plot.diff.curve.confidence <- function (DT,title,debug = FALSE){
  if(nrow(DT) == 0)
    return()
  ...
  df <- data.frame(time=DT$action_de_jeu,
                   ...
                   mise=DT$miseNorm,
                   erreurMise=DT$error.subj.diff.mise,
  ...
  p <- p + geom_step(aes(y=confidence,colour="Confiance estimée"))
  
  p <- p + geom_step(aes(y=erreurMise,colour="Erreur Mise"))
 
  p <- p + ylim(0, 1)
  p <- p + scale_colour_manual(
    values = c("Paramètre de difficulté" = "red",
               "Difficulté objective" = "red",
               "Confiance estimée" = "green",
               "Confiance" = "yellow",
               "Mise" = "blue",
               "Erreur Mise" = "gray",
               "Gagnant" = "black"))
...
#Impact du tour précédent sur la confiance
DTAll$gagnant_last <- data.table::shift(DTAll$gagnant, n=1, fill=NA, type="lag")
pas.confiant = DTAll[gagnant_last == 0]$error.subj.diff.mise
super.confiant = DTAll[gagnant_last == 1]$error.subj.diff.mise
testPlot <- as.data.table(rbind(cbind(values=pas.confiant,group=1),cbind(values=super.confiant,group=2)))
print(boxplot(values~group,data=testPlot))




plot.random.diff.curve.confidence(DTAll,"All")
#summary(glm(error.subj.diff.confiance~est.confidence.norm,data = DTM))
p <- test.profile(DTAll[action_de_jeu > 10]$error.subj.diff.mise,DTAll[action_de_jeu > 10]$est.confidence.norm,"DT")
cat("Résultat test Kendall erreur basée sur mise avec confiance estimée :", p)

plot(DTAll[action_de_jeu > 10]$error.subj.diff.mise,DTAll[action_de_jeu > 10]$est.confidence.norm)


pas.confiant = DTAll[est.confidence.norm < 0.2]$error.subj.diff.mise
super.confiant = DTAll[est.confidence.norm > 0.8]$error.subj.diff.mise
var.test(pas.confiant,super.confiant)
cat("SD des pas confiants :", sd(pas.confiant));
cat("SD des super confiants :", sd(super.confiant));

testPlot <- as.data.table(rbind(cbind(values=pas.confiant,group=1),cbind(values=super.confiant,group=2)))
print(boxplot(values~group,data=testPlot))
t <- wilcox.test(x=pas.confiant,y=super.confiant,paired=FALSE,,conf.int=T)
...
#summary(glm(error.subj.diff.confiance~est.confidence.norm,data = DTM))
p <- test.profile(DTM$error.subj.diff.mise,DTM$est.confidence.norm,"DTM")
cat("Résultat test Kendall erreur basée sur mise avec confiance estimée :", p)

plot(DTM$error.subj.diff.mise,DTM$est.confidence.norm)


pas.confiant = DTM[est.confidence.norm < 0.20]$error.subj.diff.mise
super.confiant = DTM[est.confidence.norm > 0.8]$error.subj.diff.mise
var.test(pas.confiant,super.confiant)
cat("SD des pas confiants :", sd(pas.confiant));
cat("SD des super confiants :", sd(super.confiant));
...
p <- test.profile(DTS$error.subj.diff.mise,DTS$est.confidence.norm,"DTS")

pas.confiant = DTS[est.confidence.norm < 0.2]$error.subj.diff.mise
super.confiant = DTS[est.confidence.norm > 0.8]$error.subj.diff.mise
var.test(pas.confiant,super.confiant)
cat("SD des pas confiants :", sd(pas.confiant));
cat("SD des super confiants :", sd(super.confiant));
...
cat("Résultat test Kendall erreur basée sur mise avec confiance estimée :", p)
#p <- test.profile(DTS$error.subj.diff.confiance,DTS$est.confidence.norm,"DTS")
#cat("Résultat test Kendall erreur basée sur échelle de confiance avec confiance estimée :", p)

plot.random.diff.curve.confidence(DTL,"Logical")
#summary(glm(error.subj.diff.confiance~est.confidence.norm,data = DTL))
p <- test.profile(DTL$error.subj.diff.mise,DTL$est.confidence.norm,"DTL")
cat("Résultat test Kendall erreur basée sur mise avec confiance estimée :", p)

pas.confiant = DTL[est.confidence.norm < 0.2]$error.subj.diff.mise
super.confiant = DTL[est.confidence.norm > 0.8]$error.subj.diff.mise
var.test(pas.confiant,super.confiant)
cat("SD des pas confiants :", sd(pas.confiant));
cat("SD des super confiants :", sd(super.confiant));
...
testPlot <- as.data.table(rbind(cbind(values=pas.confiant,group=1),cbind(values=super.confiant,group=2)))
print(boxplot(values~group,data=testPlot))
wilcox.test(x=pas.confiant,y=super.confiant,paired=FALSE)

#p <- test.profile(DTL$error.subj.diff.confiance,DTL$est.confidence.norm,"DTL")
#cat("Résultat test Kendall erreur basée sur échelle de confiance avec confiance estimée :", p)
...
sexe.on.error.2 <- test.profile.binary(unique(DTAll[sexe==0],by=c("IDjoueur","nom_du_jeu"))$error.subj.diff.confiance.mean, unique(DTAll[sexe==1],by=c("IDjoueur","nom_du_jeu"))$error.subj.diff.confiance.mean,"sexe.on.error.2")

sexe.on.error.m.2 <- test.profile.binary(unique(DTM[sexe==0],by=c("IDjoueur","nom_du_jeu"))$error.subj.diff.confiance.mean, unique(DTM[sexe==1],by=c("IDjoueur","nom_du_jeu"))$error.subj.diff.confiance.mean,"sexe.on.error.m.2")
sexe.on.error.s.2 <- test.profile.binary(unique(DTS[sexe==0],by=c("IDjoueur","nom_du_jeu"))$error.subj.diff.confiance.mean, unique(DTS[sexe==1],by=c("IDjoueur","nom_du_jeu"))$error.subj.diff.confiance.mean,"sexe.on.error.s.2")
sexe.on.error.l.2 <- test.profile.binary(unique(DTL[sexe==0],by=c("IDjoueur","nom_du_jeu"))$error.subj.diff.confiance.mean, unique(DTL[sexe==1],by=c("IDjoueur","nom_du_jeu"))$error.subj.diff.confiance.mean,"sexe.on.error.l.2")
