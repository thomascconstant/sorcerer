...
g.only.plots <- TRUE
cat("Seulement les courbes : ", g.only.plots)
```
...
plot.subjective.difficulty <- function(DT,selGroup,title,useConfiance){
  ...
  for(i in 1:nb.bins){
    ...
    if(nb.vals[i] > 1){
      try.res = try(test.res <- wilcox.test(jitter(subj.diff),mu = obj.diff.bin.cur,conf.int=T)) #ATTENTION utilisation méthode jitter pour éviter que les points ne se chevauchent
...
if(g.only.plots == TRUE){
  tableau.par <- par(mfrow=c(2,1))
  p1 <- recordPlot(plot.subjective.difficulty(DTAll,"good", "All tasks, good",g.use.confiance))
  plot.subjective.difficulty(DTAll,"medium", "All tasks, medium",g.use.confiance)
  plot.subjective.difficulty(DTAll,"bad", "All tasks, bad",g.use.confiance)
  par(tableau.par)
}
