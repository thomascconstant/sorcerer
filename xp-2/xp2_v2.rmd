...
est.confidence[1] = 0;
alpha <- 0.5 #Vitesse de variation de la confiance
...
```{r plot.subjective.difficulty.all.confidence.scale, echo=FALSE}
plot.subjective.difficulty(DTAll,"all", "All tasks, all groups",g.use.confiance == FALSE)
if(g.short.version == FALSE){
  plot.subjective.difficulty(DTAll,"good", "All tasks, good",g.use.confiance == FALSE)
  plot.subjective.difficulty(DTAll,"medium", "All tasks, medium",g.use.confiance == FALSE)
  plot.subjective.difficulty(DTAll,"bad", "All tasks, bad",g.use.confiance == FALSE)
}
```
...
pas.confiant = DTAll[est.confidence.norm < 0.1]$error.subj.diff.mise
super.confiant = DTAll[est.confidence.norm > 0.9]$error.subj.diff.mise
...
pas.confiant = DTM[est.confidence.norm < 0.25]$error.subj.diff.mise
super.confiant = DTM[est.confidence.norm > 0.75]$error.subj.diff.mise
var.test(pas.confiant,super.confiant)
cat("SD des pas confiants :", sd(pas.confiant));
cat("SD des super confiants :", sd(super.confiant));
...
pas.confiant = DTS[est.confidence.norm < 0.25]$error.subj.diff.mise
super.confiant = DTS[est.confidence.norm > 0.75]$error.subj.diff.mise
var.test(pas.confiant,super.confiant)
cat("SD des pas confiants :", sd(pas.confiant));
cat("SD des super confiants :", sd(super.confiant));
...
pas.confiant = DTL[est.confidence.norm < 0.25]$error.subj.diff.mise
super.confiant = DTL[est.confidence.norm > 0.75]$error.subj.diff.mise
var.test(pas.confiant,super.confiant)
cat("SD des pas confiants :", sd(pas.confiant));
cat("SD des super confiants :", sd(super.confiant));
...
```
###median time of play turn

```{r echo=FALSE}
#----------------------------------------------------------------------------------------------
#        VARIATION OF PLAY TIME (for one turn)
#----------------------------------------------------------------------------------------------

mean(DTAll$duree_tour_ms)
mean(DTM$duree_tour_ms)
mean(DTL$duree_tour_ms)
mean(DTS$duree_tour_ms)

median(DTAll$duree_tour_ms)
median(DTM$duree_tour_ms)
median(DTL$duree_tour_ms)
median(DTS$duree_tour_ms)

#cor(DTL$gagnant,DTAll$duree_tour_ms, method = c("kendall"))
