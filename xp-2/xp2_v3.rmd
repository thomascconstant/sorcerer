...
plot.subjective.difficulty <- function(DT,selGroup,title,useConfiance){
  ...
  for(i in 1:nb.bins){
    ...
    shapes.rnd.wrong[i] =  1
    shapes.dda.wrong[i] =  2
    ...
    if(nb.vals.rnd[i] > 20 && nb.vals.dda[i] > 20){
      ...
      if (class(try.res) != "try-error"){
        ...
        if(test.res$p.value <= 0.01){
          shapes.rnd.wrong[i] = 16
        }
      }
  ...
  DTPlotRnd = data.table(obj.diff=obj.diff.bin,subj.diff=subj.diff.med.rnd, shapes=shapes.rnd.dda.not.rnd)
  DTPlotDda = data.table(obj.diff=obj.diff.bin,subj.diff=subj.diff.med.dda, shapes=shapes.dda.dda.not.rnd)

  p = ggplot() + ggtitle(paste(title,"RND vs DDA"," ")) +
