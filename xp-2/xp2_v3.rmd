...
if(g.useRandomDiff == FALSE){
  DTGameXp2 = DTGameXp2[FALSE]
}
...
if(g.useDDADiff == FALSE){
  DTGameXp1 = DTGameXp1[FALSE]
}
...
#ON TRIE DTALL CORRECTEMENT
DTAll = DTAll[order(nom_du_jeu,IDjoueur,action_de_jeu)]
...
DTDescM = DTM[random_diff==0,.(type="Moteur",var=sd(miseNorm)),by=IDjoueur]
DTDescS = DTS[random_diff==0,.(type="Sensoriel",var=sd(miseNorm)),by=IDjoueur]
DTDescL = DTL[random_diff==0,.(type="Logique",var=sd(miseNorm)),by=IDjoueur]
...
print(paste("Outliers BET STANDARD DEVIATIONtype :",toString(outliers$type)))
...
```{r detect.outliers.sheeps.saved, echo=FALSE}
 ...
 DTDescM = DTM[random_diff==0,.(type="Moteur",var=max(moutons_sauves)),by=IDjoueur]
 DTDescS = DTS[random_diff==0,.(type="Sensoriel",var=max(moutons_sauves)),by=IDjoueur]
 DTDescL = DTL[random_diff==0,.(type="Logique",var=max(moutons_sauves)),by=IDjoueur]
...
DTDescM = DTM[random_diff==0,.(type="Moteur",var=max(cumulDeltaMise)),by=IDjoueur]
DTDescS = DTS[random_diff==0,.(type="Sensoriel",var=max(cumulDeltaMise)),by=IDjoueur]
DTDescL = DTL[random_diff==0,.(type="Logique",var=max(cumulDeltaMise)),by=IDjoueur]

plot.outliers(rbind(DTDescM,rbind(DTDescL,DTDescS)), "cumulDeltaMise");

outliersLoc = get.outliers(DTDescM,DTDescS,DTDescL)
outliers = merge(outliers,outliersLoc,by=c("id","type"),all=TRUE)
print(paste("Outliers :",toString(outliersLoc$id)))
print(paste("Outliers type :",toString(outliersLoc$type)))
...
```{r outliers.summary, echo=FALSE}
 ...
 print(paste("Outliers: ",toString(unique(outliers$id))))
```
...
plot.objective.diff.model.curves <- function (DT,fit,time=1,groups=T, complement="",title="title") {
  ...
  for(i in 1:nrow(DTLevel)){
    ...
    if(i==1){
      ...
      p = p + geom_path(x=DT$difficulty,y=DT$estDiff, alpha = 0.2,size=0.3,colour=DT$group.color)
    }
  }
  ...
  p = p + geom_path(x=DTDraw$difficulty,y=DTDraw$estDiff,colour="blue",size=0.6,linetype="dashed")
  ...
  p = p + geom_path(x=DTDraw$difficulty,y=DTDraw$estDiff,colour="red",size=0.5,linetype="dashed")
  ...
  title = paste(title,complement,"_")
  filename = paste(title,".png","")
  
  ggsave(filename, dpi=1200, width=11, height=7, device="png", units="cm")
  

  lab = paste("Objective Difficulty at t=",time)
  lab <- paste(lab,complement,' ')
  
  p <- p + xlab("Difficulty Variable") + ylab(lab) + theme(text = element_text(size=20))
  
  #p = p + ggtitle(title) + xlab("Paramètre de difficulté") + ylab("Difficulté objective") + theme(text = element_text(size=15))
  print(p)
}
...
#------------------------------------------------------
  DTMXp1 <- DTM[random_diff == 0]
  DTMXp2 <- DTM[random_diff == 1]
  ...
  plot.objective.diff.model.curves(DTM,fit.diff.motor,0,T,"all","motor")
  
  #xp1
  fit.diff.motor.Xp1 <- model.objective.diff(DTMXp1)
  DTMXp1 <- add.diff.obj(DTMXp1,fit.diff.motor.Xp1)
  DTMXp1 <- add.levels(DTMXp1,fit.diff.motor.Xp1)
  DTMXp1 <- add.levels.groups(DTMXp1)
  print.objective.diff.model(DTMXp1,fit.diff.motor.Xp1)
  plot.objective.diff.model.curves(DTMXp1,fit.diff.motor.Xp1,0,T,"DDA","motor")
  
  #xp2
  fit.diff.motor.Xp2 <- model.objective.diff(DTMXp2)
  DTMXp2 <- add.diff.obj(DTMXp2,fit.diff.motor.Xp2)
  DTMXp2 <- add.levels(DTMXp2,fit.diff.motor.Xp2)
  DTMXp2 <- add.levels.groups(DTMXp2)
  print.objective.diff.model(DTMXp2,fit.diff.motor.Xp2)
  plot.objective.diff.model.curves(DTMXp2,fit.diff.motor.Xp2,0,T,"Random","motor")
...
#------------------------------------------------------
  DTSXp1 <- DTS[random_diff == 0]
  DTSXp2 <- DTS[random_diff == 1]
  ...
  plot.objective.diff.model.curves(DTS,fit.diff.senso,0,T,"all","sensory")
  
  #xp1
  fit.diff.senso.Xp1 <- model.objective.diff(DTSXp1)
  DTSXp1 <- add.diff.obj(DTSXp1,fit.diff.senso.Xp1)
  DTSXp1 <- add.levels(DTSXp1,fit.diff.senso.Xp1)
  DTSXp1 <- add.levels.groups(DTSXp1)
  print.objective.diff.model(DTSXp1,fit.diff.senso.Xp1)
  plot.objective.diff.model.curves(DTSXp1,fit.diff.senso.Xp1,0,T,"DDA","sensory")
  
  #xp2
  fit.diff.senso.Xp2 <- model.objective.diff(DTSXp2)
  DTSXp2 <- add.diff.obj(DTSXp2,fit.diff.senso.Xp2)
  DTSXp2 <- add.levels(DTSXp2,fit.diff.senso.Xp2)
  DTSXp2 <- add.levels.groups(DTSXp2)
  print.objective.diff.model(DTSXp2,fit.diff.senso.Xp2)
  plot.objective.diff.model.curves(DTSXp2,fit.diff.senso.Xp2,0,T,"Random","sensory")
...
#------------------------------------------------------
  DTLXp1 <- DTL[random_diff == 0]
  DTLXp2 <- DTL[random_diff == 1]
  ...
  plot.objective.diff.model.curves(DTL,fit.diff.logic,0,T,"all","logical")
  
  #xp1
  fit.diff.logic.Xp1 <- model.objective.diff(DTLXp1)
  DTLXp1 <- add.diff.obj(DTLXp1,fit.diff.logic.Xp1)
  DTLXp1 <- add.levels(DTLXp1,fit.diff.logic.Xp1)
  DTLXp1 <- add.levels.groups(DTLXp1)
  print.objective.diff.model(DTLXp1,fit.diff.logic.Xp1)
  plot.objective.diff.model.curves(DTLXp1,fit.diff.logic.Xp1,0,T,"DDA","logical")
  
  #xp2
  fit.diff.logic.Xp2 <- model.objective.diff(DTLXp2)
  DTLXp2 <- add.diff.obj(DTLXp2,fit.diff.logic.Xp2)
  DTLXp2 <- add.levels(DTLXp2,fit.diff.logic.Xp2)
  DTLXp2 <- add.levels.groups(DTLXp2)
  print.objective.diff.model(DTLXp2,fit.diff.logic.Xp2)
  plot.objective.diff.model.curves(DTLXp2,fit.diff.logic.Xp2,0,T,"Random","logical")
...
#---------------------------------------------------------------------
if(g.short.version == FALSE){
  pvg.on.subj.diff.error.m <- test.profile(unique(DTM,by="IDjoueur")$error.subj.diff.mise, unique(DTM,by="IDjoueur")$play.video.games,"pvg.on.subj.diff.error.m")
  pvg.on.subj.diff.error.s <- test.profile(unique(DTS,by="IDjoueur")$error.subj.diff.mise, unique(DTS,by="IDjoueur")$play.video.games,"pvg.on.subj.diff.error.s")
  pvg.on.subj.diff.error.l <- test.profile(unique(DTL,by="IDjoueur")$error.subj.diff.mise, unique(DTL,by="IDjoueur")$play.video.games,"pvg.on.subj.diff.error.l")
}
```
...
#----------------------------------------------------------------------
if(g.short.version == FALSE){
  pbg.on.subj.diff.error.m <- test.profile(unique(DTM,by="IDjoueur")$error.subj.diff.mise, unique(DTM,by="IDjoueur")$play.board.games,"pbg.on.subj.diff.error.m")
  pbg.on.subj.diff.error.s <- test.profile(unique(DTS,by="IDjoueur")$error.subj.diff.mise, unique(DTS,by="IDjoueur")$play.board.games,"pbg.on.subj.diff.error.s")
  pbg.on.subj.diff.error.l <- test.profile(unique(DTL,by="IDjoueur")$error.subj.diff.mise, unique(DTL,by="IDjoueur")$play.board.games,"pbg.on.subj.diff.error.l")
}
```
...
#----------------------------------------------------------------------
if(g.short.version == FALSE){
  self.eff.on.subj.diff.error.m <- test.profile(unique(DTM,by="IDjoueur")$error.subj.diff.mise, unique(DTM,by="IDjoueur")$self.eff,"self.eff.on.subj.diff.error.m")
  self.eff.on.subj.diff.error.s <- test.profile(unique(DTS,by="IDjoueur")$error.subj.diff.mise, unique(DTS,by="IDjoueur")$self.eff,"self.eff.on.subj.diff.error.s")
  self.eff.on.subj.diff.error.l <- test.profile(unique(DTL,by="IDjoueur")$error.subj.diff.mise, unique(DTL,by="IDjoueur")$self.eff,"self.eff.on.subj.diff.error.l")
}
```
...
#----------------------------------------------------------------------
if(g.short.version == FALSE){
  risk.av.on.subj.diff.error.m <- test.profile(unique(DTM,by="IDjoueur")$error.subj.diff.mise, unique(DTM,by="IDjoueur")$risk.av,"risk.av.on.subj.diff.error.m")
  risk.av.on.subj.diff.error.s <- test.profile(unique(DTS,by="IDjoueur")$error.subj.diff.mise, unique(DTS,by="IDjoueur")$risk.av,"risk.av.on.subj.diff.error.s")
  risk.av.on.subj.diff.error.l <- test.profile(unique(DTL,by="IDjoueur")$error.subj.diff.mise, unique(DTL,by="IDjoueur")$risk.av,"risk.av.on.subj.diff.error.l")
}
```
...
```{r echo=FALSE}
#----------------------------------------------------------------------
#        INFLUENCE OF AGE AND subj.diff.error ON PLAYERS' PERFORMANCE
#----------------------------------------------------------------------
if(g.short.version == FALSE){
  age.on.subj.diff.error.m <- test.profile(unique(DTM,by="IDjoueur")$error.subj.diff.mise, unique(DTM,by="IDjoueur")$age,"age.on.subj.diff.error.m")
  age.on.subj.diff.error.s <- test.profile(unique(DTS,by="IDjoueur")$error.subj.diff.mise, unique(DTS,by="IDjoueur")$age,"age.on.subj.diff.error.s")
  age.on.subj.diff.error.l <- test.profile(unique(DTL,by="IDjoueur")$error.subj.diff.mise, unique(DTL,by="IDjoueur")$age,"age.on.subj.diff.error.l")
}
```
...
```{r echo=FALSE}
#----------------------------------------------------------------------
#        INFLUENCE OF SEX AND subj.diff.error ON PLAYERS' PERFORMANCE
#----------------------------------------------------------------------
if(g.short.version == FALSE){
  sexe.on.subj.diff.error.m <- test.profile(unique(DTM,by="IDjoueur")$error.subj.diff.mise, unique(DTM,by="IDjoueur")$sexe,"sexe.on.subj.diff.error.m")
  sexe.on.subj.diff.error.s <- test.profile(unique(DTS,by="IDjoueur")$error.subj.diff.mise, unique(DTS,by="IDjoueur")$sexe,"sexe.on.subj.diff.error.s")
  sexe.on.subj.diff.error.l <- test.profile(unique(DTL,by="IDjoueur")$error.subj.diff.mise, unique(DTL,by="IDjoueur")$sexe,"sexe.on.subj.diff.error.l")
  
  sexe.on.subj.diff.error.m.2 <- test.profile.binary(unique(DTM[sexe==0],by="IDjoueur")$error.subj.diff.mise, unique(DTM[sexe==1],by="IDjoueur")$error.subj.diff.mise,"sexe.on.subj.diff.error.m.2")
  sexe.on.subj.diff.error.s.2 <- test.profile.binary(unique(DTS[sexe==0],by="IDjoueur")$error.subj.diff.mise, unique(DTS[sexe==1],by="IDjoueur")$error.subj.diff.mise,"sexe.on.subj.diff.error.s.2")
  sexe.on.subj.diff.error.l.2 <- test.profile.binary(unique(DTL[sexe==0],by="IDjoueur")$error.subj.diff.mise, unique(DTL[sexe==1],by="IDjoueur")$error.subj.diff.mise,"sexe.on.subj.diff.error.l.2")
}
```
...
plot.subjective.difficulty <- function(DT,selGroup,title,useConfiance){
  ...
  test.dda.not.rnd.pvals = numeric(nb.bins)
  test.dda.wrong.pvals = numeric(nb.bins)
  test.rnd.wrong.pvals = numeric(nb.bins)
  
  shapes.rnd.dda.not.rnd =  numeric(nb.bins)
  shapes.dda.dda.not.rnd =  numeric(nb.bins)
  shapes.rnd.wrong =  numeric(nb.bins)
  shapes.dda.wrong =  numeric(nb.bins)
  ...
  for(i in 1:nb.bins){
    ...
    if(useConfiance)
      DTLoc = DTLoc[,.(random_diff=mean(random_diff),subj.diff.mean=mean(subj.diff.confiance)),by=IDjoueur]
    else
      DTLoc = DTLoc[,.(random_diff=mean(random_diff),subj.diff.mean=mean(subj.diff.mise)),by=IDjoueur]
    ...
    test.dda.not.rnd.pvals[i] = NA
    test.dda.wrong.pvals[i] = NA
    test.rnd.wrong.pvals[i] = NA
    ...
    shapes.rnd.dda.not.rnd[i] = 1
    shapes.dda.dda.not.rnd[i] =  2
    shapes.rnd.wrong[i] =  32
    shapes.dda.wrong[i] =  32
    ...
    if(nb.vals.rnd[i] >= 10 & nb.vals.dda[i] >= 10){
      ...
      #Difference entre RND subj et objective difficulty
      try.res = try(test.res <-  wilcox.test(subj.diff.rnd,mu=obj.diff.bin.cur,conf.int=T))
      if (class(try.res) != "try-error"){
        #print(test.res)
        #hist(subj.diff)
        test.rnd.wrong.pvals[i] = format.pval.stars(test.res$p.value)
        subj.diff.med.rnd[i] = test.res$estimate
        
        if(test.res$p.value < 0.05){
          shapes.rnd.wrong[i] = 0
        }
      }
      
      #Difference entre DDA subj et objective difficulty
      try.res = try(test.res <- wilcox.test(subj.diff.dda,mu=obj.diff.bin.cur,conf.int=T))
      if (class(try.res) != "try-error"){
        #print(test.res)
        #hist(subj.diff)
        test.dda.wrong.pvals[i] = format.pval.stars(test.res$p.value)
        subj.diff.med.dda[i] = test.res$estimate
        
        if(test.res$p.value < 0.05){
          shapes.dda.wrong[i] = 5
        }
      }
      
      #difference entre DDA et RANDOM
      if(!is.na(subj.diff.med.rnd[i]) & !is.na(subj.diff.med.dda[i]))
      {
        try.res = try(test.res <- wilcox.test(x=subj.diff.rnd,y=subj.diff.dda,conf.int=T,paired=FALSE))
        if (class(try.res) != "try-error"){
          #print(test.res)
          #hist(subj.diff)
          test.dda.not.rnd.pvals[i] = format.pval.stars(test.res$p.value)
          
          if(test.res$p.value < 0.05){
            shapes.rnd.dda.not.rnd[i] = 16
            shapes.dda.dda.not.rnd[i] = 17
          }
  ...
  print(data.table(obj.diff.bin=obj.diff.bin,subj.diff.delta.rnd.dda=subj.diff.delta.rnd.dda,n.rnd=nb.vals.rnd,n.dda=nb.vals.dda,pval=test.dda.not.rnd.pvals, shapesdda=shapes.dda.dda.not.rnd, shapesrnd=shapes.rnd.dda.not.rnd))

  DTPlotRnd = data.table(obj.diff=obj.diff.bin,subj.diff=subj.diff.med.rnd, shapes=shapes.rnd.dda.not.rnd, shapes.confidence = shapes.rnd.wrong)
  DTPlotDda = data.table(obj.diff=obj.diff.bin,subj.diff=subj.diff.med.dda, shapes=shapes.dda.dda.not.rnd, shapes.confidence = shapes.dda.wrong)
  
  #title = paste(title,"RND vs DDA"," ")

  p = ggplot()  +
        geom_abline(intercept = 0, slope = 1, color="blue")  + 
        geom_point(aes(x=DTPlotRnd$obj.diff,y=DTPlotRnd$subj.diff),alpha = 1, size = 2, shape=DTPlotRnd$shapes) +
        geom_point(aes(x=DTPlotDda$obj.diff,y=DTPlotDda$subj.diff),alpha = 1, size = 2, shape=DTPlotDda$shapes) +
        geom_point(aes(x=DTPlotDda$obj.diff,y=DTPlotDda$subj.diff),alpha = 1, size = 4, shape=DTPlotDda$shapes.confidence, color=rgb(1,0.1,0.1)) +
        geom_point(aes(x=DTPlotDda$obj.diff,y=DTPlotRnd$subj.diff),alpha = 1, size = 4, shape=DTPlotRnd$shapes.confidence, color=rgb(1,0.1,0.1)) +
        xlim(0,1)+
        ylim(0,1)+
  ...
  ggsave(paste(title,".png",""), dpi=1200, width=11, height=7, device="png", units="cm")
  
  p = p + ggtitle(title) + xlab("Difficulté objective") + ylab("Difficulté subjective") + theme(text = element_text(size=15))
   print(p)
  
  # DTPlotRnd2 = data.table(obj.diff=obj.diff.bin,subj.diff=subj.diff.med.rnd, shapes=shapes.rnd.wrong)
  # DTPlotDda2 = data.table(obj.diff=obj.diff.bin,subj.diff=subj.diff.med.dda, shapes=shapes.dda.wrong)
  # 
  # p = ggplot() + ggtitle(paste(title,"RND et DDA VS OBJ"," ")) +
  #       geom_point(aes(x=DTPlotRnd2$obj.diff,y=DTPlotRnd2$subj.diff),alpha = 1, size = 3, shape=DTPlotRnd2$shapes) +
  #       geom_point(aes(x=DTPlotDda2$obj.diff,y=DTPlotDda2$subj.diff),alpha = 1, size = 3, shape=DTPlotDda2$shapes) +
  #       xlim(0,1)+
  #       ylim(0,1)+
  #       #geom_errorbar(aes(x=DTPlotRnd$obj.diff, ymin=conf.min, ymax=conf.max), width=.01,color="red")  +
  #       geom_abline(intercept = 0, slope = 1, color="blue") +
